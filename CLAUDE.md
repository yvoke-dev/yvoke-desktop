# Yvoke - Desktop — Claude Code Project Rules
...
- **Mandatory ASDD for ALL tasks**: Always use the ASDD workflow for ALL tasks, including small / follow-up changes. No feature, refactor, behavior, or styling change may bypass ASDD phases or adversarial gates. Run `/sdd` (see `.claude/commands/sdd.md`) or read `.antigravity/sdd_protocol.md`.
- **Subagents**:
  - `desktop_implementer` (or `desktop-implementer`): writes Electron main, preload, React UI, Vanilla CSS, and Vitest code.
  - `desktop_reviewer` (or `desktop-reviewer`): read-only audit for IPC validation, memory leaks, CSP, type safety.
...
- **Classifying Failures by Prose Lets a Probe Match Itself**: `isAuthError` in `src/main/agent/ClaudeAuth.ts` matches `/credential|authentication|api key/i`, so any label passed to `readSingleReply(q, what)` containing one of those words makes its own "ended without a result" error read as an expired login — a dead subprocess then tells the user to run `claude /login`, which cannot help. `readSingleReply` throws a typed `NoReplyError` for exactly this reason: settle it by type before any regex runs, and keep probe labels free of auth vocabulary. The same trap runs the other way — a genuine Anthropic 429 body names the API key, so the rate-limit test must come BEFORE `isAuthError`, and `429` must be word-anchored (`\b429\b`) or it matches request ids and paths.
- **A New Button Variant Must Join the `button:not(...)` Exclusion Chain**: the default chrome in `src/renderer/src/styles.css` is a single `button:not(.primary):not(.secondary):not(...)` rule with ~27 `:not()` arguments, giving it specificity (0,27,1) — higher than any plain `.my-variant` class, and it appears late in the file. A variant that styles itself but is not added to that chain silently loses its border, padding, font-size and hover state, and reaching for `!important` is the symptom rather than the fix. `tests/styleTokens.test.ts` also fails the build on any `var(--token)` with no definition, since an unresolvable custom property resets the declaration to its initial value with no console error.
- **Chromium AltGr Modifier Synthesizes Ctrl+Alt**: On Windows and Linux with international keyboard layouts (German, Polish, French, Scandinavian, etc.), Chromium synthesizes the AltGr key by setting both `ctrlKey: true` and `altKey: true`. Any keybinding check targeting Ctrl (e.g. `Ctrl+Enter` to send) must explicitly guard with `!e.altKey` (i.e. `!e.altKey && (e.metaKey || e.ctrlKey) && e.key === 'Enter'`), otherwise pressing AltGr+Enter to access alternative characters or newlines prematurely triggers form submission.
- **jsdom Platform Inconsistency in Headless Tests**: In Node/jsdom test environments on macOS, `navigator.userAgent` reports `Mozilla/5.0 (darwin)...` rather than `Macintosh`. Platform detection regexes in renderer code that distinguish macOS shortcuts from Windows/Linux must include `darwin` (e.g. `/Mac|darwin|iPhone|iPad/i.test(navigator.userAgent)`), and test suites asserting platform-specific UI copy must explicitly stub `navigator.userAgent` to verify both macOS (`⌘↵`) and non-macOS (`Ctrl+↵`) paths deterministically.
- **Claude CLI Sessions Cannot Resume Across Orchestrator / Single-Agent Boundaries**: A Claude CLI session transcript on disk captures the conversation history and tool usage of the mode it started in. Resuming a single-agent session under orchestrator mode (`agent: 'orchestrator'`) causes the model to continue its single-agent persona and attempt direct tool calls instead of delegating to specialists via `Task`. Because the orchestrator root agent policy disallows non-orchestrator tools, calls fail with 'No such tool available'. Mode switches must clear `sessionId` and start a fresh session.
- **Async Action State Machines and Silent void No-Op Traps**: Any UI component setting an action state (e.g. `submitting: true`, `busy: true`, or disabling buttons) MUST define and test the exact recovery path for three distinct outcomes: (1) success, (2) rejected promise / IPC error, and (3) stale / cancelled resolution. Main-process IPC handlers for cancellable or expired actions must never return `void` on missing targets; they must return a boolean status or throw so the caller can distinguish an active resolution from a stale no-op and restore UI interactivity.
