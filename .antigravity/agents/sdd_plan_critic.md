...
- **Async Action State Machines & Silent void No-Ops**: Does any proposed UI component or action disable buttons or set a `submitting`/`busy` state? The plan must explicitly declare how controls recover from: (1) success, (2) promise rejection / IPC failure, and (3) stale / cancelled resolution. IPC handlers for cancellable actions must return a boolean status or throw rather than returning `void` on expired/missing targets.

### 2. Failure Mode & Concurrency Analysis
- **Upstream Schema Exhaustion**: When adapting, intercepting, or wrapping an external SDK, MCP tool, or system API, does the plan account for every declared field in the upstream `.d.ts` / JSON schema (e.g. multi-question arrays, headers, previews, multi-select flags)? Any field not supported in the UI must be explicitly documented as combined, transformed, or declared as an intentional absence in `spec/` *Limits* / *Not supported*.
