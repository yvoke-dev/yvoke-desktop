...
- `webSearch.allowedDomains` ships **empty**, and the allow-list is per-deployment rather than product configuration — the domains worth searching belong to whichever knowledge base is loaded. It fails closed: with `webSearch.enabled: true` and no domains configured, a search or fetch is refused rather than run against the open web, so enabling the feature means listing domains at the same time. Entries may be written as `example.com`, `https://example.com/docs` or `*.example.com` — all three mean the host and its subdomains.
...
- Being on the allow-list is not the same as being readable. A domain behind a bot-challenge — AWS WAF, Cloudflare and the like — answers a non-browser client with an empty body (`support.oneidentity.com`, for instance, returns `202` with `x-amzn-waf-action: challenge` and zero bytes), so the fetch *succeeds* and hands the model a blank page rather than an error. There is nothing the app can do about it: `canUseTool` gates the request, not the response. Check a real article on a domain before relying on it; `docs.oneidentity.com` serves fine, the KB portal does not.
